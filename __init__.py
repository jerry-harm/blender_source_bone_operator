# founction to process bone imported from source
# Copyright (C) 2024  jestar
...
def add_constraint(IK_bone: str):
    bone = bpy.context.object.pose.bones.get(IK_bone)
...
class SourceBoneOperator(bpy.types.Operator):
    bl_idname = "armature.sourcebone"
    bl_label = "Source Bone Operator"
    ...
    def execute(self, context):
        try:
            ...
            if not armature or not edit_bones:
                return {"CANCELLED"}
        except:
            return {"CANCELLED"}
        ...
        for IK_bone in IKBones:
            add_IKbone(edit_bones, IK_bone)

        bpy.ops.object.mode_set(mode='POSE')

        # add constraint
        for IK_bone in IKBones:
            add_constraint(IK_bone)
        ...
        for collection_name in BoneCollection:
            ...
            for bone_name in BoneCollection[collection_name]:
                try:
                    ...
                    print("bone collection add error")
...
def menu_func(self, context):
    self.layout.operator(SourceBoneOperator.bl_idname,
                         text=SourceBoneOperator.bl_label)


def register():
    bpy.utils.register_class(SourceBoneOperator)
    bpy.types.VIEW3D_MT_edit_armature.append(menu_func)


def unregister():
    bpy.utils.unregister_class(SourceBoneOperator)
